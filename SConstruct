...
if platform == 'windows':
    ...
    env.Append(LIBPATH=['#thirdparty/opencv-mobile/lib/windows', '#thirdparty/litert/lib'])
    env.Append(LIBS=['opencv_mobile', 'libtensorflowlite'])
elif platform == 'ios':
    env.Append(CPPDEFINES=['__APPLE__', 'IOS_PLATFORM'])
    env.Append(FRAMEWORKPATH=['#thirdparty/opencv-mobile/lib/ios'])
    env.Append(LIBPATH=['#thirdparty/litert/lib'])
    env.Append(LIBS=['opencv_mobile', 'libtensorflowlite'])
elif platform == 'macos':
    ...
    env.Append(LIBPATH=['#thirdparty/opencv-mobile/lib/android', '#thirdparty/litert/lib'])
    env.Append(LIBS=['opencv_mobile', 'libtensorflowlite'])
